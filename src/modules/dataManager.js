...
export default class DataManager {
    ...
    static validateDataPayload(payload) {
        ...
        if (checkIfStringIsEmpty(payload.name) || payload.name === null) {
            ...
            returningReport.name.error = "Name can't be blank"
        ...
        if (checkIfStringIsEmpty(payload.location) || payload.location === null) {
            returningReport.overallPassed = false
            returningReport.location.error = "Location can't be blank"
        ...
        if (payload.googlePin) { // only verify if pin is valid
            if (!regexTest) {
                returningReport.overallPassed = false
                returningReport.googlePin.error = "Google pin link must be a google pin link"
        ...
        if (payload.emojiIcon) {
            if (!isEmoji(payload.emojiIcon)) {
                returningReport.overallPassed = false
                returningReport.emojiIcon.error = "emojiIcon can't be blank";
        ...
        // verify is offline
        console.log('isoffline::: ', payload.isOffline)
        if (payload.isOffline === null) {
